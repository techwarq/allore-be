...
import { StyleMemoryService } from '../styleMemory.service'
...
function buildCreativeDirectorSystem(count: number, hasModel: boolean, hasVibeImage: boolean): string {
  return `You are an award-winning creative director for premium product ad campaigns across every
category — fashion, food, beauty, home, and lifestyle. You adapt the visual world to each brand's
own story; you do not impose one house look. In particular you never default to the generic
"trendy premium" backdrop (rooftop, city skyline, concrete ledge, bare urban street) unless the
brief explicitly calls for an urban setting.

You are given one or more reference images of a real product${hasVibeImage ? ', a real reference photo showing the vibe/setting the user wants (not the product itself)' : ''}${hasModel ? ', a reference image of the AI model who will wear/use it' : ''}, a brief from the user describing the
...
${hasVibeImage ? `
One reference image is a VIBE/SETTING reference the user explicitly picked (real photo, not
AI-generated) — it is NOT the product and nothing in it should be composited literally into the
final shot. Its only job is to ground the world/mood/lighting/color direction below in something
concrete and real instead of invented: describe exactly what you see in it (location/setting,
materials and textures, time of day, light quality and direction, color palette and grade,
atmosphere, camera style) and BASE your world/mood/lightingDirection/colorDirection fields on that
actual description, not a generic guess. If the user's text brief conflicts with it, the text brief
wins, but otherwise treat this image as the concrete source of truth for the scene.
` : ''}${hasModel ? `
The LAST reference image is the AI MODEL, not the product — a specific person's likeness to preserve
exactly (face, build, styling) in every shot. Every other reference image (product, and the vibe
reference if present) is NOT the model. Do not confuse them: never flag the model image for
isolation, and every shot's composition must include the model wearing/using/holding the product,
...
3. CREATIVE DIRECTION — do not think "how do I photograph this product?" Think: "what story, world, or
   ...
   following the user's brief${hasVibeImage ? ' and grounded in the vibe reference image described above' : ''}:
   ...
   - World: the physical scene/environment/story — specific, textured, intentional. Not "nice
     background" — an actual place with materials, weather, time of day. GROUND this world in the
     brand's OWN narrative and in how/where the product is really used or belongs. If the brief
     states where the product lives (e.g. "belongs in every home", "house breakfast", "on every
     desk", "morning ritual"), stage the shoot literally in THAT world (a real kitchen counter,
     breakfast table, home setting, etc.) — not a stylish stand-in. Do not let the brand's product
     category (e.g. a streetwear label) override the product's actual use-context; a breakfast
     product photographed for a streetwear brand still belongs in a kitchen/home, not on a rooftop.
...
function buildPromptCompilerSystem(hasModel: boolean, productImageCount: number, anchorImageNum?: number): string {
  const modelImageNum = productImageCount + 1
  const anchorClause = anchorImageNum
    ? `

STYLE ANCHOR — Image ${anchorImageNum} is a STYLE REFERENCE, not a composition input. It shows the
exact photographic look this shoot must match: its lighting quality, flash character, lens feel,
film grain/texture, and color grade. In EVERY prompt, instruct Seedream explicitly to match Image
${anchorImageNum}'s lighting, grain, lens character, and color treatment — phrase it like: "match the
photographic style of Image ${anchorImageNum}: its lighting, flash quality, film grain, lens character,
and color grade". Then immediately constrain it: "Do NOT copy Image ${anchorImageNum}'s subject,
product, person, pose, background, or composition — it is a style reference only; only the product
from Image 1${hasModel ? ` and the model from Image ${modelImageNum}` : ''} appear in the final shot."
This visual anchor is what actually carries the look — it is more reliable than any adjective, so
never drop it, but always pair it with the do-not-composite constraint above.`
    : ''
...
export class SimpleShootEngine {
  ...
  private styleMemory: StyleMemoryService
  ...
  constructor(private env: SimpleShootEnv) {
    ...
    this.styleMemory = new StyleMemoryService({
  ...
  async run(input: SimpleShootInput, stream: StreamFn): Promise<GeneratedShot[]> {
    ...
    const [textInsights, curated] = await Promise.all([
      this.findStyleReferences(query),
      this.findCuratedStyle(query),
    ])
    const styleContext = [textInsights, curated.styleText].filter(Boolean).join('\n\n')

    // Prefer the vibe image the user explicitly picked in the live Pinterest
    // vibe-picker gate; fall back to the curated Style Memory bank's
    // best-matching reference so a shoot still gets a real visual style
    // anchor even when that gate was skipped, unconfigured, or came back
    // empty. Vision reference only — never added to imageUrls below, so
    // Seedream never sees it as a literal composition input. Non-fatal: a
    // failed fetch just means the creative direction falls back to text alone.
    const effectiveVibeImageUrl = vibeImageUrl || curated.anchorImageUrl || undefined
    const vibeImage = effectiveVibeImageUrl ? await this.fetchUrlAsBase64(effectiveVibeImageUrl, stream) : null
    ...
    // Style anchor: the user's picked vibe image, injected into Seedream as a
    // VISUAL style reference (not a composition input). Text prose can't carry
    // film grain / on-camera flash / lens character / color grade — the image
    // model has to SEE the look to reproduce it. Sent LAST so its image number
    // is stable regardless of how many product/model images precede it; the
    // compiler prompt (buildPromptCompilerSystem) instructs Seedream to match
    // only its lighting/grain/lens/color and never copy its subject, product,
    // pose, or composition. This deliberately reverses the earlier "never send
    // the vibe image" choice — the compositing risk it dodged is better solved
    // with an explicit prompt instruction than by throwing the strongest style
    // signal away.
    const anchorImageUrl = vibeImage ? `data:${vibeImage.mimeType};base64,${vibeImage.base64}` : null
    const imageUrls = [...productImageUrls, ...modelImageUrls, ...(anchorImageUrl ? [anchorImageUrl] : [])]
    const anchorImageNum = anchorImageUrl ? imageUrls.length : undefined

    await stream({
      type: 'status',
      content: count > 1 ? `Writing ${direction.shots.length} shot prompts (varied camera angles)...` : 'Writing shoot prompt...',
    })
    const prompts = await this.compilePrompts(direction, hasModel, productImages.length, anchorImageNum)

    const db = getDb(this.env.DATABASE_URL)

    // Generate every shot in parallel. Each shot is an independent Seedream call
    // (~30-120s each, up to 3 retries) — running them in the old sequential loop
    // made an N-shot shoot take N× that latency for no reason. Per-shot failures
    // stay non-fatal (return null and drop out, same as the old `continue`).
    // Status/photoshoots events still carry shootIndex, so interleaved streaming
    // stays unambiguous on the client.
    await stream({
      type: 'status',
      content: prompts.length > 1 ? `Generating ${prompts.length} shots in parallel...` : 'Generating shot...',
    })
    const settled = await Promise.all(
      prompts.map((prompt, i) => this.generateAndSaveShot(i, prompt, imageUrls, direction, userId, projectId, query, db, stream))
    )
    const results = settled.filter((s): s is GeneratedShot => s !== null)
  ...
  // Generates one shot end-to-end (Seedream call → fetch → R2 → DB row → stream) and returns
  // the saved GeneratedShot, or null if this shot failed at any step. Extracted from run()'s
  // old sequential loop so the shots can be fired in parallel via Promise.all — a per-shot
  // failure here just yields null and is filtered out, so one bad shot never sinks the others.
  private async generateAndSaveShot(
    i: number,
    prompt: string,
    imageUrls: string[],
    ...
    let urls: string[]
    try {
      urls = await this.generateShotWithRetry(prompt, imageUrls, i, stream)
    } catch (err: any) {
      console.error(`[SimpleShootEngine] Shot ${i} generation failed after retries:`, err.message)
  ...
  private async developCreativeDirection(
    ...
    // Model image goes LAST so the prompt's "the LAST reference image is the
    // model" instruction (buildCreativeDirectorSystem) stays correct regardless
    // of how many product/vibe images precede it. Vibe image sits between
    // product and model — described for its scene/lighting/mood, never used
    // as a composition input for the actual generation (see run()).
    const refImages = [...productImages, ...(vibeImage ? [vibeImage] : []), ...modelImages]
  ...
  private async compilePrompts(direction: CreativeDirection, hasModel: boolean, productImageCount: number, anchorImageNum?: number): Promise<string[]> {
    const userMessage = JSON.stringify(direction, null, 2)
    const system = buildPromptCompilerSystem(hasModel, productImageCount, anchorImageNum)
  ...
  // Retries a single shot's Fal generation call on failure (timeouts, transient 5xx/queue
  // errors) before giving up — Seedream jobs occasionally exceed FalService's 120s poll
  // deadline under load, and resubmitting usually succeeds. Same attempt/backoff shape as
  // ImageGenerator.generateImage and OpenRouterTextService._attempt elsewhere in this codebase.
  private async generateShotWithRetry(prompt: string, imageUrls: string[], shootIndex: number, stream: StreamFn): Promise<string[]> {
    const maxAttempts = 3
    let lastErr: any
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        lastErr = err
        console.warn(`[SimpleShootEngine] Shot ${shootIndex} attempt ${attempt}/${maxAttempts} failed:`, err.message)
        if (attempt < maxAttempts) {
          await stream({ type: 'status', content: `Shot ${shootIndex + 1} failed, retrying (${attempt + 1}/${maxAttempts})...` })
...
to delete entirely, not something to preserve or soften.`
  ...
  // A pure absolute-score floor doesn't work with only 3 curated styles: a
  // genuinely good match (clean_minimal against its own kind of brief)
  // scored 0.39 in testing — the SAME range as a forced/mismatched top match
  // (0.32-0.36, e.g. a coastal/beach brief against these 3 unrelated
  // styles). The real signal is the MARGIN between the top match and the
  // runner-up: a brief that actually fits one curated style pulls clearly
  // ahead of the others (0.06-0.11 gap observed); a brief that fits none of
  // them leaves all 3 clustered together (0.01-0.02 gap) because there's no
  // real winner, just noise. Below this margin, skip the curated anchor
  // entirely rather than force whichever style happened to score highest —
  // e.g. a coastal/vibrant brief must NOT get anchored to
  // flash_street_editorial's dark urban night-street reference image just
  // because it edged out the other two irrelevant options. Coarse cut with
  // only 3 styles; revisit as the bank grows.
  private static readonly CURATED_STYLE_MIN_SCORE = 0.35
  private static readonly CURATED_STYLE_MIN_MARGIN = 0.04

  // Separate from findStyleReferences above — that one is a text-insight
  // search over an older knowledge base. This queries the curated Style
  // Memory / Reference Bank (scripts/pinterest-style-curator.ts's Qdrant
  // collections): given the user's brief, decides which curated visual style
  // best fits it and returns both its structured core DNA (for the Creative
  // Director prompt) and a real vetted reference image (as a style-anchor
  // fallback when the user didn't pick one via the live Pinterest vibe-picker
  // gate). Non-fatal — an empty/missing style bank or Qdrant being down just
  // means no curated anchor this shoot, same as before this feature existed.
  private async findCuratedStyle(query: string): Promise<{ styleText: string; anchorImageUrl: string | null }> {
    try {
      const candidates = await this.styleMemory.findStyles(query, 2)
      const top = candidates[0]
      if (!top) return { styleText: '', anchorImageUrl: null }

      const margin = candidates[1] ? top.score - candidates[1].score : top.score
      if (top.score < SimpleShootEngine.CURATED_STYLE_MIN_SCORE || margin < SimpleShootEngine.CURATED_STYLE_MIN_MARGIN) {
        console.log(`[SimpleShootEngine] No confident curated style match (top "${top.styleId}" score=${top.score.toFixed(3)} margin=${margin.toFixed(3)}) — skipping curated anchor, brief alone will drive direction.`)
        return { styleText: '', anchorImageUrl: null }
      }

      const dna = top.styleMemory?.core_dna
      const dnaText = dna && typeof dna === 'object'
        ? Object.entries(dna).map(([k, v]) => `${k}: ${Array.isArray(v) ? v.join(', ') : v}`).join('\n')
        : ''
      const styleText = dnaText ? `Matched curated style "${top.styleId}":\n${dnaText}` : ''

      const references = await this.styleMemory.getReferences(top.styleId, query, 1)
      const topRef = references[0]
      let anchorImageUrl: string | null = null
      if (topRef?.r2Key) {
        try {
          anchorImageUrl = await this.styleMemory.getSignedUrl(topRef.r2Key)
        } catch (err: any) {
          console.warn('[SimpleShootEngine] Failed to re-sign curated reference URL, using curation-time snapshot:', err.message)
          anchorImageUrl = topRef.signedUrl
        }
      }

      return { styleText, anchorImageUrl }
    } catch (err: any) {
      console.warn('[SimpleShootEngine] Curated style retrieval failed — continuing without it:', err.message)
      return { styleText: '', anchorImageUrl: null }
    }
  }
  ...
  // Vibe reference is an external URL (Pinterest, proxied), not an R2 key —
  // fetch and inline it directly rather than going through fetchR2AsBase64.
  // Non-fatal: falls back to no vibe image (creative direction relies on the
  // text brief alone, same as before this feature existed).
  private async fetchUrlAsBase64(url: string, stream: StreamFn): Promise<{ base64: string; mimeType: string } | null> {
    try {
      ...
      console.warn('[SimpleShootEngine] Failed to fetch vibe reference image:', err.message)
      await stream({ type: 'status', content: 'Could not load the vibe reference image, continuing without it.' })
      return null
    }
  }
