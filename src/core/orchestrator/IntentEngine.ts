...
export class IntentEngine {
  ...
  async analyze(message: string, context: AnalyzeContext): Promise<IntentPlan> {
    const prompt = new PromptBuilder()
      ...
      .add({ header: "Handoff rules", body: HANDOFF_RULES, critical: true })
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        const raw = await this.textService.generateText({
          // No model pinned here — uses whatever engine the Orchestrator injected
          // (OpenRouter qwen 3.7 flash). Hardcoding a Gemini slug would send an
          // invalid model to OpenRouter.
