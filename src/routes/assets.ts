 ...
 * All of the user's AI-generated content (photoshoots + avatars) from private_assets,
 * across every project. Powers the "Generated Items" grid on the dashboard.
 */
assets.get('/generated', async (c) => {
  const user = c.get('user');
  const db = getDb(c.env.DATABASE_URL);
  ...
  const rows = await db
    ...
    .where(and(
      eq(privateAssets.userId, user.id),
      inArray(privateAssets.type, ['photoshoot', 'avatar']),
    ))
    .orderBy(desc(privateAssets.createdAt));
  ...
  const items = await Promise.all(rows.map(async (r) => {
    ...
    try {
      ...
      // /assets/download is a public route — serve from this worker's own origin
      // so it resolves against the same R2 (local in dev, prod in prod).
      url = `${origin}/assets/download?key=${encodeURIComponent(r.r2Key)}`;
    }
    return { id: r.id, type: r.type, label: r.label ?? null, url, createdAt: r.createdAt };
  }));
